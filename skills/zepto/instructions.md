---
name: Zepto
description: Search products, manage cart, and place grocery orders on Zepto
category: integration
requiresInstance: true
auth:
  type: browser_session
  session_name: zepto
  setup_instructions: "Log in to your Zepto account via the dashboard. The agent will ask which saved address to deliver to when you place an order."
tier: installable
---

# Zepto

Search for groceries, manage your cart, and track orders on Zepto using the connected browser session.

## Setup

The user must:
1. Open a browser session for Zepto via the dashboard
2. Log in to their Zepto account
3. Ensure at least one delivery address is saved in their Zepto account

## Available Functions

### search-products
Search for products on Zepto. Params: `{ query: string, maxResults?: number }`

### add-to-cart
Add a product to the cart by searching and clicking ADD from results. Params: `{ query: string, quantity?: number }`
...
- **Smart Checkout** runs automatically before payment:
  ...
  3. Discovers all available coupons from Zepto's UI
  4. Tries applying the best coupon (up to 3 attempts)
  5. Reports savings and price breakdown
- Walks through checkout: proceed → select payment method → capture payment URL
...
- Zepto promises 10-minute delivery for most items
...
- Coupon codes are discovered from Zepto's own UI — never hardcoded
