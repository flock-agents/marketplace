...
CHECKOUT_ACTIONS=$(jq -nc '[
  ...
  {"action":"evaluate","script":"(() => { const btn = Array.from(document.querySelectorAll(\"button, a, div[role=button]\")).find(el => /pay\\s*₹|pay\\s*now|place.order|confirm/i.test(el.textContent)); if (btn) { btn.scrollIntoView({block:\"center\"}); btn.click(); return \"clicked_pay\"; } return \"no_pay_button\"; })()"},
  {"action":"wait","delay":5000}
]')

CAPTURE_CONFIG=$(_build_capture_config)

_run_checkout_flow "$PERSISTENT_ID" "${BASE_URL}/cart" "$CHECKOUT_ACTIONS" "zepto" "$CAPTURE_CONFIG"
