#!/bin/bash
# BigBasket skill helpers — sources shared helpers, adds browser-specific functions.

_HELPERS_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
source "$(cd "$_HELPERS_DIR/../../_shared" && pwd)/_helpers.sh"

BASE_URL="https://www.bigbasket.com"
LOGIN_PATTERN="bigbasket\.com/signin|bigbasket\.com/login"
SKILL_ID="${SKILL_ID:-bigbasket}"
...
_require_browser_session() {
  if [ -z "${BROWSER_SESSION:-}" ]; then
    _error_json "NO_AUTH" "No BigBasket browser session available. Connect BigBasket via the dashboard browser session."
  fi
}
...
_check_session_expired() {
  local final_url="$1"
  local session_name="${BROWSER_SESSION:-bigbasket}"
  ...
  if echo "$final_url" | grep -qiE "$LOGIN_PATTERN"; then
    curl -s -X POST "${FLOCK_API}/api/internal/browser-sessions/${session_name}/mark-outdated" \
      ...
      -d "$(jq -n --arg agent "$agent_id" --arg reason "BigBasket redirected to login page — session expired" \
            '{agentId: $agent, reason: $reason}')" >/dev/null 2>&1 || true
    _error_json "SESSION_OUTDATED" "BigBasket session has expired. Marked as outdated — user needs to re-login via the dashboard."
  fi
}
...
_crawl_url() {
  ...
  if [ "$http_code" = "403" ]; then
    ...
    if [ "$err_code" = "session_not_ready" ]; then
      _error_json "SESSION_NOT_READY" "BigBasket browser session is not ready. User needs to log in via the dashboard."
    fi
