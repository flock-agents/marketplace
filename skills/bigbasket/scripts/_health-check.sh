...
_write_health() {
  ...
  jq -nc \
    ...
    --arg session "${BROWSER_SESSION:-bigbasket}" \
    --arg status "$status" \
...
RESULT=$(_browser_navigate "$BASE_URL" 2>&1) || {
  status_json=$(_write_health "down" false false false "Could not reach BigBasket")
...
if echo "$FINAL_URL" | grep -qiE "$LOGIN_PATTERN"; then
  _mark_degraded "Health-check: BigBasket session expired — redirected to login"
...
if echo "$SEARCH_FINAL_URL" | grep -qiE "$LOGIN_PATTERN"; then
  _mark_degraded "Health-check: BigBasket session expired during search"
