---
name: BigBasket
description: Search products, manage cart, and place grocery orders on BigBasket
category: integration
requiresInstance: true
auth:
  type: browser_session
  session_name: bigbasket
  setup_instructions: "Log in to your BigBasket account via the dashboard. The agent will ask which saved address to deliver to when you place an order."
tier: installable
---

# BigBasket

Search for groceries, manage your cart, and track orders on BigBasket using the connected browser session.

## Setup

The user must:
1. Open a browser session for BigBasket via the dashboard
2. Log in to their BigBasket account
3. Ensure at least one delivery address is saved in their BigBasket account

## Available Functions

### search-products
Search for products on BigBasket. Params: `{ query: string, maxResults?: number }`
...
- Otherwise creates a new session starting at the basket page
- **Smart Checkout** runs automatically before payment:
  ...
  3. Discovers all available coupons from BigBasket's UI
  4. Tries applying the best coupon (up to 3 attempts)
  5. Reports savings and price breakdown
- Walks through checkout: proceed → address → delivery slot → payment method → capture payment URL
...
- Cart syncs between the BigBasket app and website
...
- BigBasket has a mature web app with stable selectors
- Cart contents persist across sessions on BigBasket's side
...
- Coupon codes are discovered from BigBasket's own UI — never hardcoded
