...
RESULT=$(_browser_navigate "$BASE_URL" 2>&1) || {
  status_json=$(_write_status "unreachable" "Could not reach Blinkit")
...
if echo "$FINAL_URL" | grep -qiE "$LOGIN_PATTERN"; then
  SESSION_NAME="${BROWSER_SESSION:-blinkit}"
  ...
  curl -s -X POST "${FLOCK_API}/api/internal/browser-sessions/${SESSION_NAME}/mark-outdated" \
    ...
    -d "$(jq -n --arg agent "$AGENT_ID" --arg reason "Keep-alive: Blinkit session expired — redirected to login" \
