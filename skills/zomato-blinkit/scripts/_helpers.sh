...
LOGIN_PATTERN="accounts\.google\.com|blinkit\.com/auth|blinkit\.com/login"
...
_crawl_url() {
  ...
  if [ "$http_code" = "403" ]; then
    ...
    if [ "$err_code" = "session_not_ready" ]; then
      _error_json "SESSION_NOT_READY" "Blinkit browser session is not ready. User needs to log in via the dashboard."
    fi
