#!/usr/bin/env bun
/**
 * Blinkit — keep-alive: check session health and refresh cookies.
 * Params: (none)
 */
import {
...
const SKILL_ID = process.env.SKILL_ID || "zomato-blinkit";
const BROWSER_SESSION_NAME = process.env.BROWSER_SESSION || "blinkit";
...
try {
  ...
  const statusJson = writeStatus("unreachable", "Could not reach Blinkit");
  ...
  try {
    await fetch(`${FLOCK_API}/api/internal/browser-sessions/${BROWSER_SESSION_NAME}/mark-outdated`, {
      ...
      body: JSON.stringify({
        ...
        reason: "Keep-alive: Blinkit session expired — redirected to login",
      }),
    });
  } catch {
    // ignore
