...
const labelName: string = params.name || "";

requireBrowserSession();

if (!labelName) {
  errorJson("MISSING_PARAM", "name is required");
}
...
const actions = [
  ...
  { action: "evaluate", script: `(() => { const input = document.querySelector(".xx input[type=text]") || document.querySelector("[aria-label*=\\"label name\\"] input") || document.querySelector(".Kj-JD-Jl input"); if (input) { input.value = ${labelNameJson}; input.dispatchEvent(new Event("input", {bubbles:true})); return {ok:true}; } return {ok:false,message:"Label name input not found"}; })()` },
...
const evalScript = `(() => { return JSON.stringify({ success: true, name: ${labelNameJson}, message: "Label creation attempted via browser session" }); })()`;
